...
struct AssetHistoryRecord: Hashable {
    ...
    static func parse(from assetHistory: AssetHistoryDTO) -> AssetHistoryRecord {
        AssetHistoryRecord(assetName: assetHistory.assetIdentifier, date: "01/10/2022".date()!/*assetHistory.timeStamp*/, value: assetHistory.value)
    }
    ...
    static let DollarHistoryMock: [AssetHistoryRecord] = zip(DatesMock, ValuesMock).map { AssetHistoryRecord(assetName: "EUR", date: $0, value: $1) }
}
